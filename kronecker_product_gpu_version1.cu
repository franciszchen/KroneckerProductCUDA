...
__global__ void rightKronecker_gpu1(float *A, float *B, float *C, 
	int A_height, int A_width, int B_height, int B_width, int C_height, int C_width){
	
	int tid = (threadIdx.x + blockIdx.x*blockDim.x);
	int c_col = tid%C_width ;
	int c_row = tid/C_width;

	if(tid < C_width*C_height ){

		C[c_col + c_row*C_width] = 
			A[c_col/B_width + c_row/B_height * A_width]
			* B[c_col%B_width + c_row%B_height*B_width];
...
// if use command to get matrix size
 ...
 //    }
	...
	rightKronecker_gpu1<<<(C_gpu.width*C_gpu.height-1+block_size)/block_size, block_size>>>
	...
	printf("Difference between cpu1 and gpu1:\t");
