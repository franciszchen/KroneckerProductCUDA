...
__global__ void rightKronecker_gpu2(float *A, float *B, float *C, 
	...
	int a_col = tid%A_width ;
	int a_row = tid/A_width;

	if(tid < A_width*A_height ){
		for(int j=0;j<B_height;j++){
			for(int i=0;i<B_width;i++){
				C[(i+a_col*B_width)+(j+a_row*B_height)*A_width*B_width] = 
...
// if use command to get matrix size
 ...
 //    }
	...
	rightKronecker_gpu2<<<(C_gpu.width*C_gpu.height-1+block_size)/block_size, block_size>>>
